...
void RenderTool::DrawFbo(FrameBufferInfo pFbo, int windowWidth, int windowHeight, unsigned int shaderProg)
{
    // backup the current state
    GLboolean depth_test = qglIsEnabled(GL_DEPTH_TEST);
    GLboolean blend = qglIsEnabled(GL_BLEND);
    ...
    qglBindTexture(GL_TEXTURE_2D, pFbo.ColorBuffer);
    qglColor4ub(255,255,255,255);
    qglTexCoordPointer(2, GL_FLOAT, 16, verts + 2);
    qglVertexPointer(2, GL_FLOAT, 16, verts);
    qglDrawArrays(GL_TRIANGLE_FAN, 0, 4);
...
void RenderTool::DrawFbosToFboAndDrawDatFbo(FrameBufferInfo mFbo, FrameBufferInfo* pFbos, int fboCount, int windowWidth, int windowHeight, unsigned int shaderProg)
{
    if (pFbos == NULL)
    {
        return;
    ...
    // set state
    qglBindFramebuffer(GL_FRAMEBUFFER, mFbo.Fbo);
    ...
    qglBindFramebuffer(GL_READ_FRAMEBUFFER, mFbo.Fbo);  
    qglBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0);  
    qglBlitFramebuffer(0, 0, windowWidth, windowHeight, 0, 0, windowWidth, windowHeight,  GL_COLOR_BUFFER_BIT, GL_NEAREST); 
    qglBindFramebuffer(GL_READ_FRAMEBUFFER, mFbo.Fbo);       
    qglBindFramebuffer(GL_DRAW_FRAMEBUFFER, mFbo.Fbo);
    //DrawFbo(mFbo, windowWidth, windowHeight, shaderProg);
}
